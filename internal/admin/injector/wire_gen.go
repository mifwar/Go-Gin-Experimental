// Code generated by Wire. DO NOT EDIT.

//go:generate go run -mod=mod github.com/google/wire/cmd/wire
//go:build !wireinject
// +build !wireinject

package admin

import (
	"gorm.io/gorm"
	"online-course.mifwar.com/internal/admin/delivery/http"
	admin2 "online-course.mifwar.com/internal/admin/repository"
	admin3 "online-course.mifwar.com/internal/admin/usecase"
)

// Injectors from wire.go:

func InitializedService(db *gorm.DB) *admin.AdminHandler {
	adminRepository := admin2.NewAdminRepository(db)
	adminUseCase := admin3.NewAdminUseCase(adminRepository)
	adminHandler := admin.NewAdminHandler(adminUseCase)
	return adminHandler
}
